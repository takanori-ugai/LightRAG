...
class Neo4jGraphStorage(
    ...
    init {
        ...
        if (!neo4jWorkspace.isNullOrBlank()) {
            logger.info { "Using NEO4J_WORKSPACE environment variable: '$neo4jWorkspace' (overriding '$originalWorkspace/$namespace')" }
        }
    ...
    override suspend fun initialize() {
        ...
        val authToken = if (username != null && password != null) AuthTokens.basic(username, password) else AuthTokens.none()

        // Configuration mapping
        val maxConnectionPoolSize =
            (System.getenv("NEO4J_MAX_CONNECTION_POOL_SIZE")?.toIntOrNull() ?: 100)
        val connectionTimeout =
            (System.getenv("NEO4J_CONNECTION_TIMEOUT")?.toLongOrNull() ?: 30000L) // ms
        val maxConnectionLifetime =
            (System.getenv("NEO4J_MAX_CONNECTION_LIFETIME")?.toLongOrNull() ?: 300000L) // ms
                ...
                try {
                    ...
                    if (e is ClientException && e.code() == "Neo.ClientError.Database.DatabaseNotFound") {
                        ...
                        try {
                            ...
                            logger.warn { "[$workspace] Failed to create database $db, fallback to default. Error: ${createEx.message}" }
                        }
            ...
            if (connected) {
                ...
                try {
                    driver!!.session(SessionConfig.forDatabase(database)).use { neoSession: Session ->
                        neoSession.run("CREATE INDEX IF NOT EXISTS FOR (n:`$workspaceLabel`) ON (n.entity_id)").consume()
                        logger.info { "[$workspace] Ensured B-Tree index on entity_id for $workspaceLabel in $database" }
                    }
                } catch (e: Exception) {
    ...
    private fun createFulltextIndex(
        ...
        try {
            driver.session(SessionConfig.forDatabase(database)).use { neoSession: Session ->
                ...
                if (existingIndex != null) {
                    ...
                    if (indexState == "ONLINE") {
                        logger.info { "[$workspace] Full-text index '$indexName' already exists and is online. Skipping recreation." }
                        return
                    } else {
                        logger.warn { "[$workspace] Existing index '$indexName' is not online (state: $indexState). Will recreate." }
                    }
                } else {
                ...
                if (needsRecreation || needsCreation) {
                    ...
                    } catch (cjkError: Exception) {
                        logger.warn { "[$workspace] CJK analyzer not supported: ${cjkError.message}. Falling back to standard analyzer." }
                        val createIndexQuery =
                            """
                            CREATE FULLTEXT INDEX $indexName
                            FOR (n:`$workspaceLabel`) ON EACH [n.entity_id]
            ...
            if (e.message?.contains("Unknown command") == true || e.message?.lowercase()?.contains("invalid syntax") == true) {
                logger.warn { "[$workspace] Could not create or verify full-text index '$indexName'. Use Neo4j version that supports it." }
            } else {
                logger.error { "[$workspace] Failed to create or verify full-text index '$indexName': ${e.message}" }
            }
        }
    }
    ...
    override suspend fun upsertNode(
        ...
        if (!properties.containsKey("entity_id")) {
            throw IllegalArgumentException("Neo4j: node properties must contain an 'entity_id' field")
        }
    ...
    override suspend fun getKnowledgeGraph(
        ...
        withContext(Dispatchers.IO) {
            ...
            val maxGraphNodes = (globalConfig["max_graph_nodes"] as? Number)?.toInt() ?: 1000
            ...
            driver!!.session(SessionConfig.forDatabase(database)).use { neoSession: Session ->
                if (nodeLabel == "*") {
                    ...
                    if (total > effectiveMaxNodes) {
                        logger.info { "[$workspace] Graph truncated: $total nodes found, limited to $effectiveMaxNodes" }
                    }
                }
            }
    ...
    private suspend fun robustFallback(
        ...
        while (queue.isNotEmpty() && visitedNodes.size < maxNodes) {
            ...
            if (visitedNodes.contains(currentEntityId)) continue
        ...
        withContext(Dispatchers.IO) {
            ...
            driver!!.session(SessionConfig.forDatabase(database)).use { neoSession: Session ->
                try {
                    ...
                    // Fallback
                    logger.warn { "[$workspace] Full-text search failed: ${e.message}. Falling back to standard search." }
